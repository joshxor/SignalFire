...
assert(U.browseEmptyState:IsShown() and not U.browseNext:IsShown(), "empty Browse has usable paging")
...
assert(U.browsePage == 1 and U.browseRange:GetText() == "Showing 1-8 of 8" and not U.browseNext:IsShown(), "one page is incorrect")
for number = 9, 18 do add(number) end
local pooled, snapshot = U.browseRows, U.browseSnapshot
assert(U.browsePageIndicator:GetText() == "Page 1 of 3" and U.browseRange:GetText() == "Showing 1-8 of 18"
  and U.browseRows[1].labels[1]:GetText() == "Owner 18", "page one is incorrect")
assert(U:ChangeBrowsePage(1) and U.browsePage == 2 and U.browseSnapshot == snapshot
  and U.browseRows == pooled and U.browseRange:GetText() == "Showing 9-16 of 18"
...
assert(U:ChangeBrowsePage(1) and U.browsePage == 3 and U.browseRange:GetText() == "Showing 17-18 of 18"
  and U.browseRows[1].labels[1]:GetText() == "Owner 2" and not U:ChangeBrowsePage(1), "page three is incorrect")
assert(U:ChangeBrowsePage(-1) and U.browsePage == 2, "Previous did not navigate")
...
assert(U.browsePage == 1 and U.browseRange:GetText() == "Showing 1-8 of 8", "removal did not clamp page")
