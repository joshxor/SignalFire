...
need(marketplace, "local function mkt_owner_key(value)", "canonical owner-key helper");
need(marketplace, 'string.gsub(text, "|[cC]%x%x%x%x%x%x%x%x", "")', "owner-key color wrapper removal");
need(marketplace, 'string.gsub(text, "%-.*$", "")', "owner-key realm suffix removal");
...
const currentOwner = body(marketplace, "function M:GetCurrentOwnerKey()");
need(currentOwner, "mkt_owner_key(mkt_player())", "canonical current-player identity");
need(marketplace, "ownerKey=mkt_owner_key(owner)", "canonical normalized listing ownership");
need(marketplace, "source.ownerKey ~= row.ownerKey", "migration owner-key repair accounting");
need(marketplace, "mkt_bucket_add(runtime.byOwner, row.ownerKey, row.id)", "canonical owner index construction");
...
need(ui, "self.myListingsRuntimeGeneration = nil", "My Listings runtime cache clearing");
need(ui, "self.myListingsRuntimeGeneration == runtime.generation", "My Listings runtime cache identity");
need(ui, "runtimeGeneration=runtime.generation", "My Listings cached runtime generation");
need(ui, "runtime.byOwner and runtime.byOwner[ownerKey]", "index-backed My Listings owner lookup");
const myListings = body(ui, "function U:BuildMyListingsView()");
forbid(myListings, "pairs(runtime.store.listingsById", "permanent My Listings listing scan fallback");
